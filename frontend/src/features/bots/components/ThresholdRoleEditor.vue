...
<template>
  <div class="mt-xs space-y-xs">
    <div
      class="hidden grid-cols-[10rem_minmax(0,1fr)_2.5rem] gap-xs px-xs text-xs text-text-muted tablet:grid"
    >
      <span>{{ t('botSettings.minimumThb') }}</span
      ><span>{{ t('botSettings.discordRole') }}</span
      ><span />
    </div>
    <div
      v-for="(entry, index) in entries"
      :key="index"
      class="grid gap-xs tablet:grid-cols-[10rem_minmax(0,1fr)_2.5rem]"
    >
      <input
        :value="entry[thresholdKey] ?? 0"
        type="number"
        min="0"
        step="0.01"
        class="field-control h-11"
        :placeholder="t('botSettings.minimumAmount')"
        @input="update(index, 'threshold', ($event.target as HTMLInputElement).value)"
      />
      <input
        :value="entry.roleId"
        inputmode="numeric"
        pattern="[0-9]{15,30}"
        class="field-control h-11"
        :placeholder="t('botSettings.roleId')"
        @input="update(index, 'roleId', ($event.target as HTMLInputElement).value)"
      />
      <button
        type="button"
    ...
    <p
      v-if="!entries.length"
      class="rounded-md border border-dashed border-border-subtle p-sm text-sm text-text-muted"
    >
      {{ t('botSettings.noMilestoneRolesConfigured') }}
    </p>
    <button
      type="button"
      ...
      @click="add"
    >
      <Plus :size="16" /> {{ t('botSettings.addLevel') }}
    </button>
  </div>
</template>
