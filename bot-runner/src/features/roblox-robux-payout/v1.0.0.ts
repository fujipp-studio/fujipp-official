...
async function handle(context:FeatureContext,groups:RuntimeGroup[],panelDefinitions:PayoutPanel[],queue:PayoutQueue,panels:PanelUpdater,pending:Map<string,PendingPurchase>,command:string,membershipEnabled:boolean,purchaseUsernameMaxLength:number,purchaseModalTitle:string,manualReceiptEnabled:boolean,multiPanelEnabled:boolean,interaction:Interaction){
  ...
  if(interaction.isStringSelectMenu()&&(interaction.customId===ID.group||interaction.customId.startsWith(`${ID.group}:`))){const panelKey=customIdSuffix(interaction.customId,ID.group);const panelGroups=groupsForPanel(groups,panelDefinitions,panelKey);const selected=interaction.values[0]!;if(selected===RELOAD_GROUPS){await panels.set(interaction.message,panelKey);return interaction.update(await buildPanelPayload(context,panelGroups,membershipEnabled,panelKey) as never);}return showUsernameModal(panelGroups,selected,purchaseUsernameMaxLength,purchaseModalTitle,interaction);}
...
async function postPanel(context:FeatureContext,groups:RuntimeGroup[],panelDefinitions:PayoutPanel[],panels:PanelUpdater,membershipEnabled:boolean,multiPanelEnabled:boolean,interaction:ChatInputCommandInteraction){
  ...
  const payload=await buildPanelPayload(context,panelGroups,membershipEnabled,multiPanelEnabled?panel.key:"");
...
async function buildPanelPayload(context:FeatureContext,groups:RuntimeGroup[],membershipEnabled=false,panelKey=""){
  ...
  const rows=panelActionRows(context,stock,membershipEnabled,panelKey);
...
function panelActionRows(context:FeatureContext,stock:Array<{group:RuntimeGroup;value:number|null}>,membershipEnabled:boolean,panelKey=""){
  ...
  if(membershipEnabled)rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(styledButton(context,"panel","btn_membership",customId(ID.membership,panelKey),"เช็กวันที่เข้ากลุ่ม",ButtonStyle.Primary)));
  ...
  private async refreshAll(){
    ...
    try{
      for(const [panelKey,message] of [...this.messages]){
        ...
        const payload=await buildPanelPayload(this.context,panelGroups,this.membershipEnabled,this.multiPanelEnabled?panelKey:"");
...
export interface PayoutPanel{key:string;name:string;groupKeys:string[]}
export function readPayoutPanels(value:unknown,groups:Array<Pick<RuntimeGroup,"key">>):PayoutPanel[]{
  const known=new Set(groups.map((group)=>group.key));const seen=new Set<string>();const raw=arrayConfig(value);
  const configured=raw.flatMap((item)=>{if(!isRecord(item))return[];const key=String(item.key??"").trim();const name=String(item.name??key).trim();const groupKeys=Array.isArray(item.groupKeys)?item.groupKeys.map(String).filter((groupKey)=>known.has(groupKey)):[];if(!/^[a-z0-9_-]{1,40}$/.test(key)||!name||!groupKeys.length||seen.has(key))return[];seen.add(key);return[{key,name,groupKeys:[...new Set(groupKeys)]}];});
  return configured.length?configured:raw.length?[]:[{key:"main",name:"Main Panel",groupKeys:groups.map((group)=>group.key)}];
}
